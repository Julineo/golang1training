...
func main() {
	...
	ty := big.NewRat(int64(ymax - ymin), int64(1))
	tymin := big.NewRat(int64(ymin), int64(1))
	tx := big.NewRat(int64(xmax - xmin), int64(1))
	txmin := big.NewRat(int64(xmin), int64(1))
	
	for py := 0; py < heightP; py++ {
		y := big.NewRat(int64(py), int64(heightP))
		y.Mul(y,ty)
		y.Add(y,tymin)
		for px := 0; px < widthP; px++ {
		    x := big.NewRat(int64(px), int64(widthP))
			x.Mul(x,tx)
			x.Add(x,txmin)
		    xF64,_ := x.Float64()
			yF64,_ := y.Float64()
